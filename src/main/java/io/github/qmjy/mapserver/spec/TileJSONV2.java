...
public class TileJSONV2 {
     ...
     * A semver.org style version number. When changes across tiles are introduced, the minor version MUST change.
     ...
     * When tiles change significantly, the major version MUST be increased. Implementations MUST NOT use tiles with different major versions.
     */
    private String version;

    /**
     * OPTIONAL. Default: null.
     * Contains an attribution to be displayed when the map is shown to a user. Implementations MAY decide to treat this as HTML or literal text.
    ...
    /**
     * OPTIONAL. Default: null.
     * Contains a legend to be displayed with the map. Implementations MAY decide to treat this as HTML or literal text.
     * For security reasons, make absolutely sure that this field can't be abused as a vector for XSS or beacon tracking.
     */
    ...
    /**
     * REQUIRED. An array of tile endpoints. {z}, {x} and {y} are replaced with the corresponding integers.
     * If multiple endpoints are specified, clients may use any combination of endpoints. All endpoints MUST return the same content for the same URL.
     * The array MUST contain at least one endpoint.
     */
    private String[] tiles;

    /**
     * OPTIONAL. Default: []. An array of interactivity endpoints. {z}, {x} and {y} are replaced with the corresponding integers.
     ...
     * Implementations can use this value to set the default location. If the value is null, implementations may use their own algorithm for determining a default location.
     */
